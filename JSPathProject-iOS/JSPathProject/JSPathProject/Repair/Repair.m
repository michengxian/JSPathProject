...
+ (void)startEngineWithBaseString:(NSString *)baseString
{
    ...
    context[@"__weak"] = ^id(JSValue *jsval) {
        ...
        return [[JSContext currentContext][@"_formatOCToJS"] callWithArguments:@[formatOCToJS([OCBox boxWeakObj:obj])]];
    };
    ...
    context[@"include"] = ^(NSString *filePath) {
        ...
        if (absolutePath && ![_runnedScript containsObject:absolutePath]) {
            [Repair _evaluateScriptWithPath:absolutePath];
    ...
    context[@"releaseTmpObj"] = ^void(JSValue *jsVal) {
        if ([[jsVal toObject] isKindOfClass:[NSDictionary class]]) {
            void *pointer =  [(OCBox *)([jsVal toObject][@"__obj"]) unboxPointer];
    ...
    [_context evaluateScript:baseString];
...
+ (JSValue *)_evaluateScript:(NSString *)script withSourceURL:(NSURL *)resourceURL
{
    ...
    NSString *base = [[NSUserDefaults standardUserDefaults] objectForKey:@"Repair"];
    [self startEngineWithBaseString:base];
...
+ (void)addExtensions:(NSArray *)extensions
{
    ...
    if (!_context) _exceptionBlock(@"please call [Repair startEngine]");
...
static void JPForwardInvocation(__unsafe_unretained id assignSlf, SEL selector, NSInvocation *invocation)
{
    ...
    if (!isBlock) {
        if ([slf class] == slf) {
            ...
            [argList addObject:[OCBox boxAssignObj:slf]];
            deallocFlag = YES;
        } else {
            [argList addObject:[OCBox boxWeakObj:slf]];
        }
    }
...
break; \
            ...
            case '*': {
                void *arg;
                [invocation getArgument:&arg atIndex:i];
                [argList addObject:[OCBox boxPointer:arg]];
                break;
            }
            case '#': {
                Class arg;
                [invocation getArgument:&arg atIndex:i];
                [argList addObject:[OCBox boxClass:arg]];
                break;
            }
            default: {
...
void *ret; \
id obj = formatJSToOC(jsval); \
if ([obj isKindOfClass:[OCBox class]]) { \
ret = [((OCBox *)obj) unboxPointer]; \
}
...
static id callSelector(NSString *className, NSString *selectorName, JSValue *arguments, JSValue *instance, BOOL isSuper)
{
    ...
    if (instance) {
        ...
        if (class_isMetaClass(object_getClass(instance))) {
            ...
            instance = nil;
        } else if (!instance || instance == _nilObj || [instance isKindOfClass:[OCBox class]]) {
            return @{@"__isNil": @(YES)};
        }
    }
    ...
    if (isSuper) {
        ...
        //创建一个方法替代super里面的方法，改方法的实现还是指向super方法的实现
...
break; \
            ...
            case '*':
            case '^': {
                if ([valObj isKindOfClass:[OCBox class]]) {
                    void *value = [((OCBox *)valObj) unboxPointer];
            ...
            case '#': {
                if ([valObj isKindOfClass:[OCBox class]]) {
                    Class value = [((OCBox *)valObj) unboxClass];
                    [invocation setArgument:&value atIndex:i];
                    break;
                }
            }
...
return [JSValue _methodName:result inContext:_context];    \
                ...
                case '^': {
                    void *result;
                    [invocation getReturnValue:&result];
                    returnValue = formatOCToJS([OCBox boxPointer:result]);
                ...
                case '#': {
                    Class result;
                    [invocation getReturnValue:&result];
                    returnValue = formatOCToJS([OCBox boxClass:result]);
                    break;
                }
            }
            return returnValue;
...
break;
            ...
            case '{': {
                ...
                if (end != NSNotFound) {
                    ...
                    NSDictionary *subStructDefine = [Extension registeredStruct][subStructName];
...
break;  \
            ...
            case '^': {
                ...
                void *val = [(OCBox *)dict[itemKey] unboxPointer];
                memcpy(structData + position, &val, size);
                break;
            }
            case '{': {
...
break;  \
            ...
            case '^': {
                ...
                memcpy(val, structData + position, size);
                [dict setObject:[OCBox boxPointer:val] forKey:itemKey];
                position += size;
                break;
            }
            case '{': {
...
static id formatOCToJS(id obj)
{
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSDictionary class]] || [obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSDate class]]) {
        return _autoConvert ? obj: _wrapObj([OCBox boxObj:obj]);
    }
...
static id formatJSToOC(JSValue *jsval)
{
    ...
    if ([obj isKindOfClass:[OCBox class]]) return [obj unbox];
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"]) {
            ...
            if ([ocObj isKindOfClass:[OCBox class]]) return [ocObj unbox];
            return ocObj;
...
static NSDictionary *_wrapObj(id obj)
{
    ...
    return @{@"__obj": obj, @"__clsName": NSStringFromClass([obj isKindOfClass:[OCBox class]] ? [[((OCBox *)obj) unbox] class]: [obj class])};
}
...
+ (void *)formatPointerJSToOC:(JSValue *)val
{
    id obj = [val toObject];
    if ([obj isKindOfClass:[NSDictionary class]]) {
        if (obj[@"__obj"] && [obj[@"__obj"] isKindOfClass:[OCBox class]]) {
            return [(OCBox *)(obj[@"__obj"]) unboxPointer];
        } else {
            return NULL;
        }
    } else if (![val toBool]) {
        ...
        return [((OCBox *)[val toObject]) unboxPointer];
    }
}

+ (id)formatRetainedCFTypeOCToJS:(CFTypeRef)CF_CONSUMED type
{
    return formatOCToJS([OCBox boxPointer:(void *)type]);
}

+ (id)formatPointerOCToJS:(void *)pointer
{
    return formatOCToJS([OCBox boxPointer:pointer]);
}
